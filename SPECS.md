...
{data_dir}/
  ...
  contacts/             # Per-contact context
    {name}/
      AGENTS.md
      CLAUDE.md -> AGENTS.md
  collabs/              # Collaborator submodules
    {gh-user}/
      to/               # Outgoing (shared repo submodule)
        conversations/
        drafts/
        AGENTS.md
        CLAUDE.md -> AGENTS.md
        ...
        voice.md
        .gitignore
      from/             # Incoming (their shared repo)
        conversations/
        drafts/
        AGENTS.md
        CLAUDE.md -> AGENTS.md
...
Labels in `collaborators.toml` route to `{data_dir}/collabs/{gh_user}/to/conversations/`.
...
```
corrkit collab add GITHUB_USER --label LABEL [--name NAME] [--pat] [--public] [--account ACCT] [--org ORG]
```

Creates a private GitHub repo (`{org}/to-{gh_user}`), initializes with AGENTS.md/README.md/voice.md/conversations/drafts, adds as git submodule under `collabs/{gh_user}/to/`. Updates `collaborators.toml`.
...
```
corrkit collab rename OLD NEW [--rename-repo]
```

`git mv` the directory, update collaborators.toml.
`--rename-repo`: also rename the GitHub repo via `gh repo rename`.

Searches for the directory in `{data_dir}/collabs/{name}/to/` and `{data_dir}/collabs/{name}/from/`.

### 5.16 collab reset

```
corrkit collab reset [NAME] [--no-sync]
```
...
4. Add as git submodule at `collabs/{gh_user}/to/`
...
1. Find directory in `collabs/{name}/to/` or `collabs/{name}/from/`
2. `git mv` old → new
...
Manual: create file in `{data_dir}/drafts/` or `{data_dir}/collabs/{gh_user}/to/drafts/`.
